...
const ImageUploader = () => {
  ...
  return <div className="flex flex-col items-center">
      <div className={`w-full max-w-md border-2 border-dashed rounded-lg p-6 text-center ${previewUrl ? 'border-emerald-300' : 'border-gray-300'}`} onDrop={handleDrop} onDragOver={handleDragOver}>
        {!previewUrl ? <div className="space-y-4">
            ...
            <div className="text-gray-600">
              ...
              <input ref={fileInputRef} type="file" accept="image/jpeg, image/png" onChange={handleImageSelect} className="hidden" id="image-upload" />
              <label htmlFor="image-upload" className="mt-2 inline-block px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-md cursor-pointer hover:bg-emerald-700">
                Chọn ảnh
              </label>
            </div>
            <p className="text-xs text-gray-500">
              Chấp nhận định dạng JPG, PNG
            </p>
          </div> : <div className="relative">
            <img src={previewUrl} alt="Preview" className="max-h-64 mx-auto rounded" />
            <button onClick={resetImage} className="absolute top-2 right-2 bg-gray-800 bg-opacity-50 text-white rounded-full p-1 hover:bg-opacity-70">
              <XIcon className="h-4 w-4" />
            </button>
          </div>}
      </div>
      {previewUrl && !ocrResult && <button onClick={processImage} className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-md flex items-center" disabled={isLoading}>
          {isLoading ? <>
      ...
      {ocrResult && <div className="w-full max-w-md mt-6">
          ...
          {suggestedProducts.length > 0 && <div className="mt-6">
              <h3 className="font-medium mb-3">Sản phẩm gợi ý:</h3>
              <div className="space-y-4">
                {suggestedProducts.map(product => <div key={product.id} className="bg-white rounded-lg shadow p-3 flex">
